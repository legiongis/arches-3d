define([
    ...
    var viewModel = function(params) {
		...
		var parseDisplay = function(descvalue) { 
			...
			if (descvalue.startsWith("None") || descvalue.startsWith("[")) {
				...
				var descvaluehtml = '<div class="media-left"><img class="img-fluid" src="' + descvalue1[0]['url'] + '" height=60></div>';	
		...
		this.popupData = ko.computed(function() {
            if (self.hoverData()) {
				...
				//look for the map_popup value and run it through the parseDisplay function in search.js - AML
				var latesthovval = self.hoverData[Object.getOwnPropertySymbols(self.hoverData)[0]];
				//console.log(latesthovval.map_popup());
				var cleanhovmap = parseDisplay(latesthovval.map_popup());
				self.hoverData[Object.getOwnPropertySymbols(self.hoverData)[0]]['cleanmap'] = cleanhovmap;
            }
            if (self.clickData()) {
                self.getMarkup(self.clickData);
				//look for the map_popup value and run it through the parseDisplay function in search.js - AML
				var latestval = self.clickData[Object.getOwnPropertySymbols(self.clickData)[0]];
				//console.dir(latestval);
				//console.log(latestval.map_popup());
				var cleanmap = parseDisplay(latestval.map_popup());
				self.clickData[Object.getOwnPropertySymbols(self.clickData)[0]]['cleanmap'] = cleanmap;
				//console.dir(self.clickData);
			}
